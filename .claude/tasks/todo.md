...
### High Priority
- [ ] **BUG-SCROLL-1**: Chat window fails to auto-scroll on new message — after sending, view stays static; must scroll manually to see sent message (labels: ux, chat)
- [ ] **BUG-ICONS-1**: Icons rendering as horizontal lines (Unicode/glyph parse failure) — all icons throughout the app appear as plain horizontal lines; missing icon font or glyph loading failure (labels: ui, rendering, unicode, fonts)

### Medium Priority
- [ ] **BUG-SCROLL-BTN-1**: Scroll-to-bottom button flickering and unresponsiveness — button flickers on appearance/hover and click registration is inconsistent (labels: ui, chat, controls)

### Low Priority
- [ ] **BUG-HOVER-1**: Contact name/avatar hover state causes UI flicker — hovering over contact name/avatar triggers visual flicker as inline action buttons attempt to appear (labels: ui, chat, hover-effects)
