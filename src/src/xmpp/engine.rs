            ...
            maybe_event = client.next() => {
                match maybe_event {
                    ...
                    Some(ev) => {
                        ...
                        handle_client_event(ev, event_tx, &mut outbox, &mut reconnect_attempt, &mut sm, &mut blocking_mgr, &mut mam_mgr, &mut catchup_mgr, &mut own_jid_str).await;
                    }
                }
            }
    ...
    match ev {
        tokio_xmpp::Event::Online { bound_jid, .. } => {
            ...
            *own_jid_str = bound_jid.to_string();
            ...
            // (server-wide MAM query without `with` filter gets recent messages)
            let (_, mam_query) = catchup_mgr.start("__server__", None);
            ...
            tracing::info!("mam: triggered post-connect catchup query");
        ...
        tokio_xmpp::Event::Stanza(el) => {
            ...
            dispatch_stanza(el, event_tx, blocking_mgr, sm, outbox, mam_mgr, catchup_mgr, own_jid_str).await;
        }
    }
}
    ...
    match el.name() {
        ...
        "message" => {
            ...
            if let Some(inner) = extract_carbon(&el, "sent") {
                ...
                if !body.is_empty() {
                    let _ = event_tx.send(XmppEvent::MessageReceived(IncomingMessage {
                        ...
                        from: own_jid_str.to_string(), // we sent it
                        body,
                    })).await;
                    tracing::debug!("carbons: forwarded sent message to {bare_to}");
                }
                return;
            }
            ...
            if let Some(inner) = extract_carbon(&el, "received") {
                ...
                tracing::debug!("carbons: forwarded received message");
                return;
            }
...
fn extract_carbon<'a>(el: &'a Element, direction: &str) -> Option<Element> {
