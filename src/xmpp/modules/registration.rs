...
impl RegistrationManager {
    ...
    pub fn build_registration_form_submit(id: &str, form: Element) -> Element {
        let query = Element::builder("query", NS_REGISTER).append(form).build();
...
mod tests {
    ...
    fn test_build_registration_submit() {
        let el = RegistrationManager::build_registration_submit(
            "reg2",
            "alice",
            "secret",
        ...
        let query = el.get_child("query", NS_REGISTER).unwrap();
        assert_eq!(
            query.get_child("username", NS_REGISTER).unwrap().text(),
            "alice"
        );
        assert_eq!(
            query.get_child("password", NS_REGISTER).unwrap().text(),
            "secret"
        );
        assert_eq!(
            query.get_child("email", NS_REGISTER).unwrap().text(),
            "alice@example.com"
        );
    }
}
